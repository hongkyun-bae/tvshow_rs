...
namespace MyMediaLite.ItemRecommendation
{
	...
	public class BPRMF : MF, IFoldInItemRecommender
	{
        ...
        ArrayList wableintv_list;
        ArrayList wintv_list;
        ArrayList all_wableintv_list;
		double pos_threshold, comp_threshold;
		...
		public override void Train()
		{
            ...
            pos_threshold = 0.1;
            StreamReader sr_ep = new StreamReader("../../../data/500000_Epi_Prop_competable.base");
            ...
            sr_ep = new StreamReader("../../../data/500000_Epi_Prop_competable.base");
            ...
            Console.WriteLine("Confidence (500000): " + elapsedTime.TotalSeconds);
		...
		public override void Iterate()
		{
			...
			{
				if (WithReplacement)
					IterateWithReplacementUniformUser();    // 수행
                else
			...
			{
				if (WithReplacement)
					IterateWithReplacementUniformPair();    // 221221
				else
		...
		protected virtual void IterateWithReplacementUniformUser()
		{
			...
			//int sub_events = positive_list.Count / 10;

			Console.WriteLine("Number of iterations: " + num_pos_events);	

            int idx = 0, user_id = 0, pos_item_id = 0, neg_item_id = 0;
            UserItemDate target_pair = null;
            ArrayList competable_list = null;
			string negatives;
			//string negatives_diffs;
            ...
            //for (int pos_id = (process_num-1) * sub_events; pos_id < process_num * sub_events; pos_id ++)
            for (int pos_id = 0; pos_id < num_pos_events; pos_id++)
            {
				...
				UserItemEP ep_pair = (UserItemEP)positive_list[pos_id];
				...
				/* Open file to save negative episodes of each target episode with writing mode */
				//StreamWriter streamWriter = new StreamWriter("E:\\hongkyun\\TVshow\\wableIntv-based\\negatives_" + pos_threshold + "_" + comp_threshold + "\\Epi_Prop_negatives_" + idx + ".base");
				negatives = null;

                /* Determine candidates of competable episodes for the corresponding user */
                //StreamReader sr_wableintv = new StreamReader("E:\\hongkyun\\TVshow\\watchable\\Epi_Prop_wable_intv_" + user_id + ".base");
                StreamReader sr_wableintv = new StreamReader("D:\\Deadpool_backup\\TVshow\\watchable\\Epi_Prop_wable_intv_" + user_id + ".base");
                wableintv_list = new ArrayList();

                //int num_rows = 0;
                string line = "";
                while ((line = sr_wableintv.ReadLine()) != null)
                {
                    string[] row = line.Split('\t');
					int cur_idx = int.Parse(row[0]);
					int uid = int.Parse(row[1]);
					...
					{
						DateTime date_time = Convert.ToDateTime(dt[j]);
						dt_list.Add(date_time);
					}

					//UserItemDate date_pair = new UserItemDate(num_rows, uid, tid, dt_list);
					UserItemDate date_pair = new UserItemDate(cur_idx, uid, tid, dt_list);

					//if (idx == num_rows)
					if (idx == cur_idx)
					{
						//Console.WriteLine("IDX: " + idx + ", num_rows: " + num_rows);
						target_pair = date_pair;
					}
					else
						wableintv_list.Add(date_pair);
				}
                sr_wableintv.Close();

                /* Determine competable episodes from the list of watchable episodes */
                competable_list = new ArrayList();

				DateTime tp_start = (DateTime)target_pair.DateList[0];
				DateTime tp_end = (DateTime)target_pair.DateList[1];

				foreach (UserItemDate wable_pair in wableintv_list)
                {
					DateTime wable_start = (DateTime)wable_pair.DateList[0];
					DateTime wable_end = (DateTime)wable_pair.DateList[1];

					TimeSpan wable_diff = wable_end - wable_start;
					double threshold_amount = wable_diff.TotalSeconds * comp_threshold;

					DateTime overlapped_start = wable_start;
					DateTime overlapped_end = wable_end;
					double overlapped_amount = 0;

					if (threshold_amount < 0)
					{
						Console.WriteLine("ERROR: Invalid amount of competitive threshold");
						continue;
					}

					// Skip non-overlapped wable episodes
					if ((wable_start.CompareTo(tp_end) == 1) || (wable_end.CompareTo(tp_start) == -1))
						continue;

					// Compute overlapped interval
					if (wable_start.CompareTo(tp_start) == -1)
						overlapped_start = tp_start;
					if (wable_end.CompareTo(tp_end) == 1)
						overlapped_end = tp_end;

					TimeSpan overlapped_diff = overlapped_end - overlapped_start;
					overlapped_amount += overlapped_diff.TotalSeconds;

					if (overlapped_amount < 0)
					{
						Console.WriteLine("ERROR: Invalid amount of overlapped interval");
						continue;
					}

					if (overlapped_amount >= threshold_amount)   // for the case of alpha==0.0
					{
						neg_item_id = wable_pair.ItemID;
						//UpdateFactors(user_id, pos_item_id, neg_item_id, true, true, update_j);

						negatives += neg_item_id;   // Indices of negative episodes
						negatives += ',';
					}
				}
				//streamWriter.WriteLine(negatives);
				//streamWriter.Close();
			}

            DateTime finishTime = DateTime.Now;
            TimeSpan elapsedTime = finishTime - startTime;
            Console.WriteLine("Finding losers (500000): " + elapsedTime.TotalSeconds);

            Console.WriteLine("Total computed rows: " + i);
		}
		...
		/// </summary>
		protected virtual void IterateWithReplacementUniformPair()  // Get random pairs and identify the pairs with non-competition
		{
            //int num_pos_events = Feedback.Count;
            int num_pos_events = positive_list.Count;
            ...
            foreach (UserItemEP positive_pair in positive_list)
            {
                ...
                int user_id = positive_pair.UserID;
                int pos_item_id = positive_pair.ItemID;
                int neg_item_id = -1;
                SampleOtherItem(user_id, pos_item_id, out neg_item_id);                

                streamWriter.WriteLine("{0}\t{1}\t{2}\t{3}", idx, user_id, pos_item_id, neg_item_id);     //굳이 write 안 시켜도 될 것 같음
            }
            streamWriter.Close();
